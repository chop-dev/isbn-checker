...
public class ISBNChecker {
    ...
    public static String checkISBN(String isbn) {
        ...
        if (len == 10) {
            output = isbn10(isbn, output ,len);

        // Determine if there's 13 Digits in the ISBN
        } else if (len == 13) {
            output = isbn13(isbn, output, len);
        }
        return output;
    }

    // 10 Digit ISBN Check Method
    public static String isbn10(String isbn, String output ,int len) {
    ...
    // 13 Digit ISBN Check Method
    public static String isbn13(String isbn, String output ,int len) {
        // Calculate the sum of the ISBN Digits
        int sum = 0;
        for (int i = 0; i < len; i++) {
            int num = 1;
